...
if(!empty($_SESSION['cart'])) {
      ...
      // Display body with a form and table.

      echo '<form action="JeansCart.php" method="post"><table><tr><th
      ...
      while($row = mysqli_fetch_array($r, MYSQLI_ASSOC)) {
              ...
              $total += $subtotal;
              // Display the rows.
              echo "<tr><td>{$row['item_name']}</td><td>{$row['item_desc']}</td> <td><input type=\"text\" size=\"3\"
              name=\"qty[{$row['item_id']}]\" value=\"{$_SESSION['cart'][$row['item_id']]
              ['quantity']}\"></td>
      ...
      mysqli_close($dbc);
      // Display the total.
      echo '<tr><td column="5" style"text-align:right">Total = ' .
      number_format($total, 2) . '</td></tr></table><input type="submit" name="submit"
      value="Update My Cart"></form>';
    } else {
...
// Navigation links.
echo '<p><a href="./Jeans.php">Shop</a> | <a href="JeansCheckout.php?
total=' . $total . '">Checkout</a></p>';
?>
