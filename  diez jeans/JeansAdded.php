  ...
  <! --- 
     *******************************************
     ...
     if(isset($_POST['id'])) {
      ...
      // Retrieve item data from the 'shop' database table with the passed id.
      $q = "SELECT * FROM jeans.products WHERE item_id = $id";
      $r = mysqli_query($dbc, $q);

      if(mysqli_num_rows($r) == 1) {
      $row = mysqli_fetch_array($r, MYSQLI_ASSOC);
      
      // Check to see if cart already contains a product with the passed id.
      
      
        if(isset($_SESSION['cart'][$id])) {

      // Add one more of this product if so.
      
      $_SESSION['cart'][$id]['quantity']++;
        echo '<p>Another ' . $row["item_name"] . ' has been added to your cart</p>';
        } else {
      
        // Or add the product if not already in the cart.
